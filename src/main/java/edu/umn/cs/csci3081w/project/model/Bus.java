...
/**
 * This is the class for the bus to load necessary data.
 */
public class Bus implements BusObserver {
  ...
  /**
   * Check if the Bus trip is completed.
   *
   ...
   * @return if the passenger is loaded successfully
   */
  ...
  public boolean move() {
    ...
    if (!isTripComplete() && distanceRemaining <= 0) {
      ...
      if (passengersHandled >= 0) {
        ...
        // we don't get to count excess distance
        // towards next cBus testBus = TestUtils.createBus();stop
        distanceRemaining = 0;
      }
      //switch to next stop
      toNextStop();
  ...
  /**
   * Move the bus and update its data.
   */
  public void update() {
    move();
    updateBusData();
  ...
  /**
   * Get the bus next stop.
   *
   * @return data of next stop
   */
  ...
  /**
   * Get total amount of passenger on the bus.
   *
   * @return passenger size on the bus
   */
  ...
  /**
   * Get maximum amount of passenger the bus can fit.
   *
   * @return number of max capacity on bus
   */
  ...
  /**
   * Get the bus out going route.
   *
   * @return data of out going route
   */
  ...
  /**
   * Get the bus incoming route.
   *
   * @return data of incoming route
   */
